...
Changes that introduce new features or modify existing behavior must include documentation updates within the `/docs` folder. README updates alone are not sufficient — the `/docs` folder is the primary documentation source. Documentation should follow the style guide at `/docs/contribute/doc-style-guide.md`.
