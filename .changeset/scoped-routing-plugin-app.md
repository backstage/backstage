...
A page's sub-pages are now ordinary routes one level below the page, matched by the app rather than by a route tree inside the page. Any routing library can therefore host a page with tabs, not only React Router, and a sub-page whose content uses a different routing library attaches its own router rather than inheriting the one its page uses. A page's tabs and a tab's content can be rendered by different libraries.
