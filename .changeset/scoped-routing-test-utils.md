...
Passing your own `navigate` now observes the call instead of taking the place of the mock's own behavior, so the target check, the location update and the `location$` emission all still happen around it. Options such as `replace` and `state` reach it exactly as the caller wrote them, and a one-argument call stays a one-argument call, so assertions on the arguments stay accurate.
