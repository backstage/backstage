...
- A static route now wins over a parameterized one that could also match, regardless of how many segments each has, so a page registered at a fixed path is no longer shadowed by another plugin's wildcard.
