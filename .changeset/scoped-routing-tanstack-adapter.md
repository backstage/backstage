...
The adapter reports that opaque React Router children are unsupported — pages must use route descriptors (or keep a React Router adapter). TanStack page history blockers register into the shared framework blocker seam so adapter and chrome navigation share the same policy.
