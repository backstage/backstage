...
Adds `useAppNavigate` / `useOptionalFrameworkNavigate` so shared plugin code can use framework navigation when a navigation controller is present and fall back to React Router when it is not (dual-path for new and old frontend apps). App chrome can read location with `useFrameworkLocation` or call the navigation controller API directly. Page routers can declare capabilities; contracts stay stable across concrete base path changes under the same page pattern; subpages receive their own scoped contract and resolve an empty router input the same way pages do.
