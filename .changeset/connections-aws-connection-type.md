...
`@backstage/integration-aws-node` gained an alpha `createAwsCredentialsManagerFromConnection` export that creates an `AwsCredentialsManager` backed by the connections system; the existing configuration-based setup is unchanged.
