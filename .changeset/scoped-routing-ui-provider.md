...
`BUIProvider` accepts an optional `useRouter` hook

A host app can provide a hook that returns navigation, a plain href resolver and the current browser pathname together as one `BUIRouter`, and route every descendant BUI component through a single navigation authority without needing an ambient React Router context. The host capability is shared across independently loaded copies of Backstage UI. When it is omitted, standalone and old frontend system usage continues to adapt the surrounding React Router v6 context.

The hook runs at each consuming component and its href resolver receives targets as written, so a target written inside a page, whether `#tab`, `?tab=readme` or a relative path, reaches the host app intact and is resolved against that page. Resolved hrefs and the current pathname both include the deployment basename. Header navigation, tabs and links read the active pathname from the same hook, so rendering and navigation cannot end up configured from different routers.
...
**Affected components:** Link, ButtonLink, Tabs, Menu, Header, PluginHeader, Card, TagGroup, Table
