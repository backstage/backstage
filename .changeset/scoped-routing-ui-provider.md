...
Setting `useHref` makes it the only thing that resolves a link target: descendant components hand their target over as it was written instead of resolving it against React Router first. A target written inside a page — `#tab`, `?tab=readme`, or a relative path — therefore reaches the host app intact and is resolved against that page, wherever the component rendering it sits in the React Router tree.
