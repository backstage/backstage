...
MkDocs itself is healthy. mkdocs-material enters end-of-life in November 2026, with critical fixes only until then and nothing after. Its creator is pivoting to **Zensical**, a next-generation static site generator that has some backwards compatability with `mkdocs.yml` natively and preserves Material's CSS classes in "classic" mode.
...
| `pymdown-extensions`        | Py Markdown ext (13 exts)     | Features listed as native; unclear if via pymdownx or own impl |
| `plantuml-markdown`         | Py Markdown ext               | Not mentioned by Zensical , tested lightly on local            |
| `markdown-graphviz-inline`  | Py Markdown ext               | Not mentioned by Zensical, tested lightly on local             |
| `mdx_truly_sane_lists`      | Py Markdown ext               | Not mentioned by Zensical, tested lightly on local             |
...
This is a requirement so that Zensical-based generation can maintain feature parity. There can be further discussion on whether or not this belongs on the image or should be done programatically through code.
...
Pluggable generator registry, per-entity engine selection via catalog annotations, frontend selector map, CLI serve support, `engine` field in metadata. See [BEP-0015: TechDocs Engine Agnosticism](../0015-techdocs-engine-agnosticism/) and [Future Work](#future-work).
