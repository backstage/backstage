...
description: >
  ...
  extension blueprints), backend (createBackendPlugin, createBackendModule), external API integration,
  proxy config, catalog providers/processors, scaffolder actions, and real-world patterns from
  backstage/community-plugins, RoadieHQ/roadie-backstage-plugins, and backstage-plugin-opsgenie. Use
  whenever working on a Backstage plugin or module, plugin structure, workspace setup, naming conventions,
  API design, entity cards/pages, catalog integration, external service integration, app-config patterns,
  or contributing to community-plugins or Roadie repos. Also trigger for installing, registering, or
  debugging any @backstage package, or building a plugin similar to existing open-source ones. Trigger
  even for "Backstage plugin", "build a plugin", or any Backstage-related concept.
---
...
- **Lockfile out of date after adding a workspace package** → Run `yarn install --mode=update-lockfile` and commit `yarn.lock`. CI in hardened mode rejects lockfile mutations.
- **Version mismatch in workspace deps** → When a sibling package (e.g. `-common`) has been released to a new version on `main`, rebase your branch before pushing. Check `"version"` in the dep's `package.json` matches the `"^x.y.z"` range in consumers.
- **Prettier CI failure** → Always run `yarn prettier --write <changed-files>` before committing. Never format entire directories (risks changing unrelated files).

---

## 12. Pre-Push Checklist

**Always verify these before pushing changes to a plugin workspace:**

1. **Lockfile is in sync** — If you added/removed packages or workspace entries, run `yarn install --mode=update-lockfile` and commit `yarn.lock`
2. **Prettier passes** — Run `yarn prettier:check` (or `yarn prettier --write` on changed files). Fix all warnings.
3. **Version references are aligned** — If you depend on sibling workspace packages (e.g. `-common`, `-node`), ensure the `^x.y.z` range in `package.json` covers the current version of that package on `main`
4. **Rebase onto latest main** — Especially if other PRs have merged that touch `yarn.lock` or bump versions
5. **No stale exports** — If you moved/deleted code (e.g. extracting a module), grep for leftover import paths (`grep -r "old/path"`)
6. **API reports updated** — If public API surface changed, run `yarn build:api-reports` and commit `.api.md` files
7. **Commits are signed off** — All commits need `Signed-off-by:` (use `git commit -s`)
