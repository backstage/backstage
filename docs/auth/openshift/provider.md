...
- `additionalScopes`: (optional): By default only the scope `user:info` is required. Additional scopes can be found in ["User scopes"](https://docs.redhat.com/en/documentation/openshift_container_platform/latest/html-single/authentication_and_authorization/index#tokens-scoping-about_configuring-internal-oauth).
