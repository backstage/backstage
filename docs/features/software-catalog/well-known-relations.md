...
### `deploysToEnvironment` and `environmentHasDeployment`

A relation noting that a Component is deployed to a certain Environment.

This relation is used to tag a Component with Environment specific information.

This relation is generated by the `spec.deploysToEnvironment` of the Component in question.
