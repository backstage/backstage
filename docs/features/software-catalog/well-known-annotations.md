...
### backstage.io/techdocs-skip-build
...
When this annotation is set to `true`, the TechDocs backend will not attempt to
build documentation for this entity.

This annotation is intended to be used with `techdocs.builder` set to `local`. In
this configuration, the TechDocs backend will attempt to build documentation for
entities without the `backstage.io/techdocs-skip-build: true` annotation. Entities
with the annotation are expected to have their docs generated by an external process,
as suggested for when `techdocs.builder` is set to `external`.

This approach captures both the ease-of-use of local TechDocs builds (users can commit
their docs to their repository, and have them built without any setup), with the
flexibility of external TechDocs builds (users are free to customize how their docs are built).
