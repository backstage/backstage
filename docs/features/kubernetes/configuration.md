...
kubernetes:
  ...
  clusterLocatorMethods:
    ...
    - type: 'gke'
      ...
      skipTLSVerify: true
    - type: 'watcher'
      labelSelector: 'backstage.io/remote-cluster-secret=true'
      labelClusterName: 'backstage.io/remote-cluster-name'
```
...
Used by the `ClustersWatcherLocator` to watch multi-cluster kubernetes secrets
...
metadata:
  labels:
    backstage.io/remote-cluster-secret: 'true'
    backstage.io/remote-cluster-name: cluster-name
  name: cluster-name-mcsa-token-vrv4q
  namespace: chosen-namespace
type: Opaque
```

The label **backstage.io/remote-cluster-secret: "true"** identifies a secret as
a remote cluster holder and **backstage.io/remote-cluster-name** label provides
the cluster name. Both labels should be configurable when configuring the
'watcher' '**clusterLocatorMethods** in **app-config.yaml**.

##### How to link a remote cluster
...
[script](https://github.com/backstage/backstage/blob/master/contrib/kubernetes/multi-cluster/README.md)
