...
### BREAKING: Restrictions on API factory overrides in new frontend system

The ability for plugins to override API factories from other plugins has been restricted. If an API is provided by a plugin, no other plugin can provide the same API. Only modules for the owning plugin can override the API. This restriction applies to both core Utility APIs (provided by the `app` plugin) and APIs from other plugins.

This is a breaking change that will cause app startup to fail if you have plugins attempting to override APIs from other plugins. To migrate, you need to convert these overrides to use either:

- A plugin override (for the owning plugin)
- A module with `pluginId` set to the owning plugin

For example, if you were overriding a core API like `AlertApi` from a plugin, you now need to create an `app` module instead:

```ts
const alertApiModule = createFrontendModule({
  pluginId: 'app',
  extensions: [
    ApiBlueprint.make({
      params: defineParams =>
