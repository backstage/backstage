...
We use `zod` and `@backstage/catalog-model` in our example below. To install them, run the following from your Backstage root directory, replacing the path with your permission policy module's location:

```bash title="from your Backstage root directory"
yarn --cwd <your-permission-policy-module-path> add zod@3 @backstage/catalog-model
```

```ts title="<your-permission-policy-module-path>/src/permissionRules.ts"
...
2. Create a `catalogPermissionRules.ts` file in the `packages/backend/src/extensions` folder with the following content. Replace the package name with the one generated by `yarn new`:
   ...
   import { isInSystemRule } from '<your-permission-policy-module-package-name>';
