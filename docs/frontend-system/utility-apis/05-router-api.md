...
For testing components that use routing, `@backstage/frontend-test-utils` provides `MockMemoryRouterApi` and `MockBrowserRouterApi`, along with `TestMemoryRouterProvider` and `TestBrowserRouterProvider` for wrapping components under a router context. The `renderInTestApp` helper handles this automatically:
