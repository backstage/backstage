...
const PipelineNavigationStyles = {
  titleBox: {
    position: 'relative',
...
class Header extends Component {
  ...
  componentSwitcherFragment() {
    ...
    const { workflows = [], dataEndpoints = [] } = component;
    const workflowCount = (workflows && workflows.length) || 0;
    const dataEndpointCount = (dataEndpoints && dataEndpoints.length) || 0;

    const model = {
      workflows: workflows,
      dataEndpoints: dataEndpoints,
      componentId: component.id,
      ownerName: component.owner && component.owner.name,
      hasWorkflows: Boolean(workflowCount),
      hasDataEndpoints: Boolean(dataEndpointCount),
    };

    return (
      <ComponentWorkflowsEndpointsToolbar
        type={type}
        pageModel={model}
        styles={PipelineNavigationStyles}
        pageTitle={title}
      />
    );
  }
  ...
  render() {
    ...
    if (typeof pageTitle !== 'string' && !env.isProduction) {
          ...
          {theme => (
            <header style={style} className={classes.header}>
              ...
              <div className={classes.leftItemsBox}>
                ...
                {this.componentSwitcherFragment()}
              </div>
