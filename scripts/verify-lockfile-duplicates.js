...
async function findLockFiles() {
  ...
  if (!files.length) {
    // Locate all lock files that are in the root or in an immediate subdirectory
    files = await globby(['*/yarn.lock', 'yarn.lock'], {
      cwd: projectRoot,
    });
  }
...
async function main() {
  ...
  for (const lockFile of lockFiles) {
    ...
    if (failed) {
      if (!fix) {
        const command = `yarn dedupe --cwd ${lockFile.directoryRelativeToProjectRoot}`;
