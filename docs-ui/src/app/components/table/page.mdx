...
Use `defaultWidth` with `fr` units for the initial proportional layout, then add `minWidth` and `maxWidth` in pixels to prevent columns from becoming too narrow or too wide.
...
Set `isResizable` to let users resize a column with a pointer or keyboard. It can be combined with `defaultWidth`, `minWidth`, and `maxWidth` to configure the initial size and constraints.

<CodeBlock code={tableResizableColumnsSnippet} />
