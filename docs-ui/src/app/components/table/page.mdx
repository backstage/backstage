...
Set `isResizable` to let users resize a column with a pointer or keyboard. Use `defaultWidth` with `fr` units for the initial proportional layout, then add `minWidth` and `maxWidth` in pixels to constrain the available sizes.
