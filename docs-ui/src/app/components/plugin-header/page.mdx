import { PropsTable } from '@/components/PropsTable';
import { CodeBlock } from '@/components/CodeBlock';
import { Snippet } from '@/components/Snippet';
import {
  WithAllOptionsAndTabs,
  WithAllOptions,
  WithHeader,
  WithBreadcrumbs,
  WithBreadcrumbsAndTabs,
} from './components';
import { headerPropDefs } from './props-definition';
import {
  usage,
  simple,
  defaultSnippet,
  withTabs,
  withHeader,
  withBreadcrumbs,
  withBreadcrumbsAndTabs,
} from './snippets';
import { PageTitle } from '@/components/PageTitle';
import { Theming } from '@/components/Theming';
import { PluginHeaderDefinition } from '../../../utils/definitions';
import { ChangelogComponent } from '@/components/ChangelogComponent';

<PageTitle
  title="PluginHeader"
  description="A plugin header with icon, title, custom actions, and navigation tabs."
/>

<Snippet py={4} preview={<WithAllOptionsAndTabs />} code={defaultSnippet} />

## Usage

<CodeBlock code={usage} />

## API reference

<PropsTable data={headerPropDefs} />

## Examples

### Simple plugin header

<Snippet py={4} preview={<WithAllOptions />} code={simple} open />

### Plugin header with tabs

Tabs use React Router and highlight automatically based on the current route.

<Snippet preview={<WithAllOptionsAndTabs />} code={withTabs} open />

### Plugin header with breadcrumbs

Use breadcrumbs for hierarchical navigation within the header.

<Snippet preview={<WithBreadcrumbs />} code={withBreadcrumbs} open />

### Plugin header with breadcrumbs and tabs

Breadcrumbs and tabs can be combined for multi-level navigation.

<Snippet
  preview={<WithBreadcrumbsAndTabs />}
  code={withBreadcrumbsAndTabs}
  open
/>

### Plugin header with Header

Combine with [Header](/components/header) for multi-level navigation.

<Snippet preview={<WithHeader />} code={withHeader} open />

<Theming definition={PluginHeaderDefinition} />

<ChangelogComponent component="plugin-header" />
