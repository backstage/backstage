 ...
 *
 * List of tasks:
 * - Add repo_url or edit_uri if it does not exist.
...
export const patchMkdocsYmlPreBuild = async (
  ...
  await patchMkdocsFile(mkdocsYmlPath, logger, mkdocsYml => {
    ...
    if (
      !('repo_url' in mkdocsYml) &&
      !('edit_uri' in mkdocsYml) &&
      !('edit_uri_template' in mkdocsYml)
    ) {
      ...
      if (result.repo_url || result.edit_uri_template) {
        mkdocsYml.repo_url = result.repo_url;
        mkdocsYml.edit_uri_template = result.edit_uri_template;

        logger.info(
          `Set ${JSON.stringify(
            result,
          )}. You can disable this feature by manually setting 'repo_url' or 'edit_uri' or 'edit_uri_template' according to the MkDocs documentation at https://www.mkdocs.org/user-guide/configuration/#repo_url`,
        );
        return true;
      }
    }
