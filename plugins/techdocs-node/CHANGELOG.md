...
- 32723a0: Improved handling of mapping-style Markdown extension configuration.

## 1.14.7

### Patch Changes

- 9f76ea4: **BREAKING**: TechDocs generation now removes MkDocs plugins outside the built-in list of permitted plugins and logs a warning before continuing the build. To retain additional plugins, review and add them to `techdocs.generator.mkdocs.dangerouslyAllowAdditionalPlugins`, or supply them through `dangerouslyAllowAdditionalPlugins` when creating the generator directly. Plugins configured through `defaultPlugins` are also permitted. Generation stops if the MkDocs configuration cannot be read, parsed, or updated.
