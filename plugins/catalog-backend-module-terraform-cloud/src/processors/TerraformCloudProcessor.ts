...
export class TfCloudReaderProcessor implements CatalogProcessor {
  static fromConfig(config: Config, logger: Logger) {
    return new TfCloudReaderProcessor(config, logger);
  }
  ...
  async readLocation(
    ...
    try {
      ...
      const modules = await this.fetchModuleData(
        `https://app.terraform.io/api/v2/organizations/${location.target}/registry-modules`,
        token,
      );
      for (let i = 0; i < modules.data.length; i++) {
        ...
        const moduleEntity = buildModuleEntity(
        ...
        for (
          let resourceIndex = 0;
          resourceIndex < moduleResources.root.resources.length;
          resourceIndex++
        ) {
          resourceArray.push(
            `Resource:${moduleResources.root.resources[resourceIndex].name}`,
          );
...
function buildResourceEntity(
  ...
  const resourceEntity: Entity = {
    ...
    spec: {
      type: resourceType,
      dependencyOf: [`Component:${dependency}`],
      owner: tfOrg,
    },

    metadata: {
      name: resourceName,
      tags: [`${providerName}`, `${moduleName}-tf-module`],
    },
  };
