 ...
 */
export type ClustersWatcherLocatorConfig = {
  /**
...
export class ClustersWatcherLocator implements KubernetesClustersSupplier {
  private readonly clusterDetails = new Map<string, ClusterDetails>();
  private readonly logger: Logger;
  private readonly locatorConfig: ClustersWatcherLocatorConfig;
  ...
  constructor({
    ...
    logger: Logger;
    locatorConfig: ClustersWatcherLocatorConfig;
  }) {
    this.logger = logger;
  ...
  }): ClustersWatcherLocator {
    const locatorConfig = readFromConfig(clusterLocatorConfig);
    const autoLocator = new ClustersWatcherLocator({ logger, locatorConfig });
    autoLocator.start();
    return autoLocator;
  }
  ...
  private logPoolSize() {
    this.logger.info(
      `Automatically discovered clusters pool: ${this.clusterDetails.size}`,
    );
  }
}
