...
describe ('OpenAPISpecParser', () => {
    ...
    const v3Spec = readFileSync(path.resolve(__dirname, "./test_specs/petstore_v3.yml"), 'utf-8');
    const v3Document:OpenAPIV3.Document = parse(v3Spec)

    const v31Spec = readFileSync(path.resolve(__dirname, "./test_specs/petstore_v31.yml"), 'utf-8');
    const v31Document:OpenAPIV3_1.Document = parse(v31Spec)

    const v2Spec = readFileSync(path.resolve(__dirname, "./test_specs/petstore_v2.yml"), 'utf-8');
    ...
    describe('getV2SpecText', () => {
        it('returns expected Parser Text', () => {
            const indexableStringArray = parser.getV2SpecText(v2Document as OpenAPIV2.Document).filter(x=>x)
    ...
    describe('getV3SpecText', () => {
        it('returns expected Parser Text', () => {
            const indexableStringArray = parser.getV3SpecText(v3Document as OpenAPIV3.Document).filter(x=>x)
