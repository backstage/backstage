...
export async function createRouter(
  ...
  router.get('/getChecklists', async (req, res) => {
    try {
      ...
      const statusArr = prevData.map(item => item.isDone);
      if (statusArr?.length) showModal = statusArr.some(val => !!!val);
      if (!checklists?.length) showModal = false;
      if (checklists?.length) {
        checklists.forEach(value => {
          ...
          const prevItem = prevData.find(
            it => it.checklist_uid === value.checklist_uid,
          );
