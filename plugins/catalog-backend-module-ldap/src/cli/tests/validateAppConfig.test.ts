...
import { basicAppConfigWithLdap } from '../mocks/basicAppConfigWithLdap';
...
describe('validateAppConfig', () => {
  it('should tell us if we already add an LDAP processor', () => {
    expect(validateAppConfig(basicAppConfig)).toEqual(false);
  });
  it("should tell us if we don't add an LDAP processor", () => {
    expect(validateAppConfig(basicAppConfigWithLdap)).toBe(true);
  });
});
