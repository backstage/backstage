 ...
 */
describe('checkIfWeImportThePlugin', () => {
  it('should tell us if we already add an LDAP processor', () => {
    expect(checkIfWeImportThePlugin(basicCatalogFile)).toEqual(false);
  });
  it("should tell us if we don't add an LDAP processor", () => {
    expect(checkIfWeImportThePlugin(basicCatalogFileWithProcessor)).toBe(true);
  });
});
