 ...
 */
describe('checkIfWeAddTheProcessor', () => {
  it('should tell us if we already add an LDAP processor', () => {
    expect(checkIfWeAddTheProcessor(basicCatalogFile)).toEqual(false);
  });
  it("should tell us if we don't add an LDAP processor", () => {
    expect(checkIfWeAddTheProcessor(basicCatalogFileWithProcessor)).toBe(true);
  });
});
