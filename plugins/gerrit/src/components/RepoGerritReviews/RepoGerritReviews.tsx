...
const GetGerritReviews = () => {
  ...
  if (entity.kind === 'System') relationType = RELATION_HAS_PART;
  else if (entity.kind === 'Group') relationType = RELATION_OWNER_OF;
  else repoView = true;
  ...
  const flattenedValue = value.flatMap(
    (item: Array<{ [key: string]: string }>) => item,
  );
