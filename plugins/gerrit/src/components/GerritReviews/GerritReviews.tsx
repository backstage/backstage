...
export const showReviewers = (reviewers: any) => {
  if (reviewers.REVIEWER && reviewers.REVIEWER.length) {
    const transformedReviewList = reviewers.REVIEWER.map(
      (d: any) => ` ${d.name}`,
    ).reduce((acc: Array<string>, currVal: string) => {
...
const findRelatedJIRAs = (inputText: string): JSX.Element => {
  ...
  */
  ...
  const jiraBaseURL: string =
    'https://eteamproject.internal.ericsson.com/browse/';
...
export const getSize = (insertions: 0, deletions: 0) => {
  ...
  return (
    <OverflowTooltip
      text={size}
      title={`Added ${insertions}, Removed ${deletions} lines`}
    />
  );
};
...
export const DenseTable = ({
  ...
  const gerritChangeColumns: TableColumn[] = [
    {
      ...
      render: (gerritChange: any) => (
        <Link
          to={`${baseUrl}/c/${gerritChange.fullProject}/+/${gerritChange.id}`}
        >
    ...
    },
    {
      ...
      render: (gerritChange: any) => (
        <Link
          to={`${baseUrl}/plugins/gitiles/${gerritChange.fullProject}/+/refs/heads/${gerritChange.branch}`}
        >
    ...
    },
    {
      ...
      render: (gerritChange: any) => (
        <Tooltip
          title={`${gerritChange.total_comment_count} comments / ${gerritChange.unresolved_comment_count} unresolved comments`}
        >
  ...
  const data = gerritChangesList.map(gerritChange => {
    return {
      ...
      code_review: setGerritLabelValueAndName(
        gerritChange.labels,
        'Code-Review',
      ),
      commit_message: setGerritLabelValueAndName(
        gerritChange.labels,
        'Commit-Message',
      ),
  ...
  if (repoView) {
    title = <Link to={`${cloneUrl}/${tableTitle}`}>{tableTitle}</Link>;
    gerritRepoHistory = (
  ...
  return (
    <InfoCard title={title} subheader={gerritRepoHistory}>
      <Table
        ...
        localization={{
          toolbar: { searchPlaceholder: 'Search Subject or ID' },
        }}
      />
    </InfoCard>
  );
...
const GetGerritCurrentUser = () => {
  ...
  const { value, loading, error } = useAsync(async () => {
    const gerritResponse = await fetch(
      `${await proxyBackendBaseUrl}/gerrit/a/accounts/self`,
    );
...
export const GerritReviewCard = ({
  ...
  const {
    ...
    loading,
    error,
  } = useAsync(async (): Promise<GerritChange[]> => {
    const proxyBackendBaseUrl = discoveryApi.getBaseUrl('proxy');
    const gerritResponse = await fetch(
      `${await proxyBackendBaseUrl}/gerrit/a/${request}`,
    );
    if (!gerritResponse.ok) {
      throw new Error(gerritResponse.statusText);
    }
  ...
  <Page themeId="tool">
    ...
    <Content>
      <Grid container spacing={3} direction="column">
        <Grid item>
          <GerritReviewCard
            request="changes/?O=81&S=0&n=50&q=is:open+owner:self"
            tableTitle="Open Reviews"
          />
        </Grid>
        <Grid item>
          <GerritReviewCard
            request="changes/?O=81&S=0&n=50&q=is:open+reviewer:self+-owner:self"
            tableTitle="Incoming Reviews"
          />
        </Grid>
        <Grid item>
          <GerritReviewCard
            request="changes/?O=81&S=0&n=50&q=is:closed+owner:self"
            tableTitle="Closed Reviews"
          />
        </Grid>
      </Grid>
    </Content>
