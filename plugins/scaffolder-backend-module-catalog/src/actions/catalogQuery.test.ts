...
import { createCatalogQueryAction } from './catalogQuery';
...
describe('catalogQuery', () => {
  ...
  const action = createCatalogQueryAction({ catalog });
  ...
  it('should call catalog', async () => {
    ...
    catalog.queryEntities.mockResolvedValueOnce({
      items: [ENTITY1],
      totalItems: 1,
      pageInfo: {},
    });
    ...
    expect(output).toHaveBeenCalledWith('totalItems', 1);
  });

  it('should handle exceptions', async () => {
    catalog.queryEntities.mockRejectedValueOnce(
