...
import { AzureDevOpsTechDocsEventSubscriber } from './AzureDevOpsTechDocsEventSubscriber';
...
describe('AzureTechDocsEventSubscriber', () => {
  ...
  const subscriber = AzureDevOpsTechDocsEventSubscriber.fromConfig(config, {
    logger,
    tokenManager,
    catalogClient,
  });
  ...
  it(`should only support the ${AZURE_DEVOPS_TOPIC_REPO_PUSH} topic`, async () => {
    const support = subscriber.supportsEventTopics();
    expect(support).toEqual([AZURE_DEVOPS_TOPIC_REPO_PUSH]);
  });

  it(`should handle the ${AZURE_DEVOPS_TOPIC_REPO_PUSH} topic`, async () => {
