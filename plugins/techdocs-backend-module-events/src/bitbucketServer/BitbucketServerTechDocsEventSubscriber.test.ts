 ...
 */
import {
  BITBUCKET_SERVER_TOPIC_REPO_PUSH,
...
import { BitbucketServerTechDocsEventSubscriber } from './BitbucketServerTechDocsEventSubscriber';
...
describe('BitbucketServerTechDocsEventSubscriber', () => {
  ...
  const subscriber = BitbucketServerTechDocsEventSubscriber.fromConfig(config, {
    logger,
    tokenManager,
    catalogClient,
  });
  ...
  it(`should only support the ${BITBUCKET_SERVER_TOPIC_REPO_PUSH} topic`, async () => {
    const support = subscriber.supportsEventTopics();
    expect(support).toEqual([BITBUCKET_SERVER_TOPIC_REPO_PUSH]);
  });

  it(`should handle the ${BITBUCKET_SERVER_TOPIC_REPO_PUSH} topic`, async () => {
    const entities: Entity[] = [];
    mockedGetMatchedEntities.mockResolvedValue(entities);

    const event: EventParams = {
      topic: BITBUCKET_SERVER_TOPIC_REPO_PUSH,
      eventPayload: {
        repository: {
