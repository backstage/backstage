 ...
 *
 * TanStack's `__TSR_index` / `__TSR_key` fields project the framework's private
 * current-entry metadata. Keys therefore remain stable across remounts and
 * traversals, and indices agree with every other first-party adapter.
 *
...
export function createTanStackHistory(
  ...
  function splitScope(appPathname: string): PageScope | undefined {
    const match = matchPath(options.routePattern, appPathname, false);
    if (!match) {
      return undefined;
    }
    const base = match.pathnameBase;
  ...
  function resolveMetadata(
    ...
    if (action?.type === 'REPLACE') {
      return { ...latestMetadata, action: 'REPLACE' };
    }
    if (action?.type === 'PUSH' || sourceLocation !== appHistory.location) {
      const index = latestMetadata.index + 1;
      return {
        action: 'PUSH',
        key: `tanstack-${fallbackKey++}`,
        index,
        length: index + 1,
        canGoBack: index > 0,
      };
  ...
  const ensureSubscription = () => {
    ...
    subscription = appHistory.location$.subscribe(loc => {
      ...
      if (action) {
        ...
        const delta = metadata.index - previousIndex;
        let traversal: HistoryNotifyAction;
        if (delta === -1) {
          traversal = { type: 'BACK' };
        } else if (delta === 1) {
          traversal = { type: 'FORWARD' };
        } else {
          traversal = { type: 'GO', index: delta };
        }
        notify(traversal);
      }
    });
  ...
  const performNavigate = (path: string, state: unknown, replace: boolean) => {
    ...
    // above (if attached) has already fired and consumed pendingAction. Do not
    // synthesize its metadata a second time in that case. Resyncing below
    // covers the case where no subscriber is attached yet.
    const handledBySubscription = pendingAction === undefined;
    pendingAction = undefined;
