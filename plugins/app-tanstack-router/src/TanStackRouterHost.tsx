...
import { usePageMountResolver } from '@internal/frontend';
...
function createTanStackScopedRouter(
  ...
  resolveMount: ReturnType<typeof usePageMountResolver>,
 ...
 *
 * Renders `children` untouched when no `AppHistoryApi` is registered. The
 * caller has already established that there is a page mount, but that is only
 * half of what scoping needs — without an app history there is no location to
 * project and nothing to build a router from, which is the same "nothing to
 * scope to" as a missing mount. Deciding that here rather than relying on the
 * caller is what keeps the adapter inert in a dual-mode package: both halves
 * are checked on the way in, so neither can be assumed by the other.
 *
 * @internal
 */
  ...
  const scoped = useMemo(
    () =>
      appHistory
        ...
        : undefined,
    [appHistory, routePattern, createPageRouter, resolveMount],
  );
