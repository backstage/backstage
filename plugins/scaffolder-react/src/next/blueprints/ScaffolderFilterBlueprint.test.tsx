 ...
 */
import { ScaffolderFilterBlueprint } from './ScaffolderFilterBlueprint';
import {
  coreExtensionData,
  createExtension,
...
describe('ScaffolderFilterBlueprint', () => {
  it('should create an extension with sane defaults', () => {
    const extension = ScaffolderFilterBlueprint.make({
      params: {
        loader: async () => <div />,
      },
    });
  ...
  it('should allow overriding of inputs and config', async () => {
    const extension = ScaffolderFilterBlueprint.makeWithOverrides({
      name: 'test-name',
      inputs: {
    ...
    const mockExtension = createExtension({
      attachTo: { id: 'scaffolder-filter:test-name', input: 'mock' },
