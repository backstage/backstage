 ...
 */
import { CatalogContentHeaderItemBlueprint } from './CatalogContentHeaderItemBlueprint';
import {
  coreExtensionData,
  createExtension,
...
describe('CatalogContentHeaderItemBlueprint', () => {
  it('should create an extension with sane defaults', () => {
    const extension = CatalogContentHeaderItemBlueprint.make({
      params: {
        loader: async () => <div />,
      },
    });
  ...
  it('should allow overrding of inputs and config', async () => {
    const extension = CatalogContentHeaderItemBlueprint.makeWithOverrides({
      name: 'test-name',
      inputs: {
    ...
    const mockExtension = createExtension({
      attachTo: { id: 'catalog-content-header-item:test-name', input: 'mock' },
