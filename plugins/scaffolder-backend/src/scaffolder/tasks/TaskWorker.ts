...
export class TaskWorker {
  ...
  protected truncateParameters(parameters: JsonObject) {
    const truncatedParameters: JsonObject = { ...parameters };
    Object.keys(truncatedParameters).forEach(key => {
      const value = truncatedParameters[key]?.toString();
      const auditMaxLength = this.config
