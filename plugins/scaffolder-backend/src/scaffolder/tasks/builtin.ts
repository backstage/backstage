...
export function createFetchCookiecutterAction(options: {
  ...
  return {
    ...
    async handler(ctx) {
      ...
      const fetchUrl = ctx.parameters.url ?? '.';
      ...
      if (ctx.baseUrl?.startsWith('file://')) {
        const basePath = ctx.baseUrl.slice('file://'.length);
        ...
        await fs.copy(srcDir, templateContentsDir);
      } else {
        let readUrl;

        if (ctx.baseUrl) {
          const integration = integrations.byUrl(ctx.baseUrl);
          if (!integration) {
            throw new InputError(
              `No integration found for location ${ctx.baseUrl}`,
            );
          }
          ...
          await res.dir({ targetDir: templateContentsDir });
        }
      }
