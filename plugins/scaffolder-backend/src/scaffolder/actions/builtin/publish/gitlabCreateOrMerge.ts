...
export function createGitlabCreateOrMergeAction(options: {
  ...
  return createTemplateAction<{
    ...
    description:
      'Initializes a git repository of the content in the workspace, and publishes it to GitLab if the repo does not exist else create a new merge request with new template.',
    schema: {
      input: {
        type: 'object',
        required: ['repoUrl'],
    ...
    async handler(ctx) {
      ...
      if (!doesRepoWithSameNameAlreadyExists) {
        ...
        try {
          ...
          // ctx.output('catalogInfoUrl', `${repoContentsUrl}/catalog-info.yaml`);
        } catch (e) {
