...
export const createPublishGitlabMergeRequestAction = (options: {
	...
	return createTemplateAction<GitlabMergeRequestActionInput>({
	  ...
	  async handler(ctx) {
        ...
        const destinationBranch = ctx.input.destinationBranch? ctx.input.destinationBranch + formatedTimestamp(): `backstage_${formatedTimestamp()}`;
		...
		const defaultBranch: any = await api.Projects.show(ctx.input.projectid).then((projectJSON) => {
