...
export async function createRouter(
  ...
  router
    .get(
      ...
      async (req, res) => {
        ...
        try {
          ...
          const templateRef = `${template.kind}:${template.metadata.namespace || 'default'
    ...
    .post('/v2/dry-run', async (req, res) => {
      ...
      try {
        ...
        const templateRef: string = `${template.kind}:${template.metadata.namespace || 'default'
          }/${template.metadata.name}`;
  ...
  async function authorizeTemplate(
    ...
    if (!isSupportedTemplate(template)) {
      throw new InputError(
        `Unsupported apiVersion field in schema entity, ${(template as Entity).apiVersion
        }`,
      );
    }
