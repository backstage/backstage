...
describe('GoldenPathsEntitiesProcessor', () => {
  describe('validateEntityKind', () => {
    it('validates the entity kind', async () => {
      const processor = new GoldenPathsEntitiesProcessor();
    ...
    it('generates relations for component entities', async () => {
      ...
      expect(emit).toHaveBeenCalledWith({
        ...
        relation: {
          ...
          type: 'ownerOf',
          target: { kind: 'GoldenPath', namespace: 'default', name: 'n' },
        },
      });
      expect(emit).toHaveBeenCalledWith({
        type: 'relation',
        relation: {
          source: { kind: 'GoldenPath', namespace: 'default', name: 'n' },
          type: 'ownedBy',
