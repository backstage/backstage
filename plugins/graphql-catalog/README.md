Missing features: `filters`, `viewer`, some other catalog client methods, like `nodes`
...
export const myModule = createModule({
  ...
  typeDefs: loadFilesSync(
    resolvePackagePath('backend', 'src/graphql/my-module.graphql'),
  ),
  resolvers: {
    Query: {
      hello: () => 'world',
...
export const extensionsModule = createModule({
  ...
  dirname: resolvePackagePath('backend', 'src/graphql'),
  typeDefs: loadFilesSync(
    resolvePackagePath('backend', 'src/graphql/extensions.graphql'),
  ),
});
```
...
export default printSchemaWithDirectives(
  transformSchema(
    loadFilesSync(resolvePackagePath('backend', 'src/graphql/*.graphql')),
  ),
);
```
...
interface Repository
  @extends(interface: "Entity", when: "kind", is: "Repository") {
