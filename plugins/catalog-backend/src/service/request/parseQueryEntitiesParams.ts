...
export function parseQueryEntitiesParams(
...
): Omit<QueryEntitiesRequest, 'authorizationRequest' | 'limit'> {
  const fields = parseEntityTransformParams(params);

  if (params.cursor) {
    const decodedCursor = decodeCursor(params.cursor);
    const response: Omit<QueryEntitiesCursorRequest, 'authorizationRequest'> = {
  ...
  const response: Omit<QueryEntitiesInitialRequest, 'authorizationRequest'> = {
    fields,
    filter,
    orderFields,
    fullTextFilter: {
