 ...
 */
export class CatalogProcessingEngine {
  ...
  async read(location: LocationSpec): Promise<CatalogRefreshResult> {
    const { rulesEnforcer, logger } = this.options;

    const output: CatalogRefreshResult = {
      entities: [],
      errors: [],
      locations: [],
    };
