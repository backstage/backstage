  ...
  await knex.schema.hasColumn('refresh_keys', 'id').then(async hasCol => {
    if (!hasCol) {
      if (knex.client.config.client.includes('sqlite3')) {
        ...
        // recreate the original table with a primary key
        // add columns, column types, and relationships from original table
        ...
        await knex.schema.createTable('refresh_keys', table => {
          ...
          table
            ...
            .onDelete('CASCADE')
            .comment(
              'A reference to the entity that the refresh key is tied to',
            );
          table
            .string('key')
            .notNullable()
    ...
    if (!hasCol) {
      if (knex.client.config.client.includes('sqlite3')) {
        ...
        await knex.schema.table('tmp_relations', table => {
          table.dropIndex(
            'source_entity_ref',
            'relations_source_entity_ref_idx',
          );
          table.dropIndex(
    ...
    if (!hasCol) {
      if (knex.client.config.client.includes('sqlite3')) {
        ...
        await knex.schema.createTable('search', table => {
          table.comment(
            'Flattened key-values from the entities, for filtering',
          );
          table
            .increments('id', { primaryKey: true })
            .comment('Primary key to distinguish unique lines from each other');
      ...
      // await knex.schema.raw(`INSERT INTO refresh_keys (entity_id, key) SELECT entity_id, key FROM tmp_refresh_keys`);
      await knex.schema.raw(
