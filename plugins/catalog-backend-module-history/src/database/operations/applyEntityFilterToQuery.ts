...
// This file is copied and lightly modified from catalog-backend.
...
export function applyEntityFilterToQuery(options: {
  ...
  // This only applies filtering to entries that are related to any entity ID at
  // all in the first place.
  return targetQuery.where(outer => {
    outer.orWhereNull(onEntityIdField);
    outer.orWhere(inner =>
      applyInStrategy(filter, inner, onEntityIdField, knex, false),
    );
  });
}
