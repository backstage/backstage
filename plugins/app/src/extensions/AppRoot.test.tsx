...
// BUIProvider installs. Each consumer calls the host hook in its own scope.
...
describe('AppRoot', () => {
  ...
  // Sign-in content must receive the same routing integration as page content.
  // Use a full app because renderTestApp finalizes without a sign-in page.
  ...
  // The host hook handed to BUIProvider is called at each anchor's own
  ...
  // current page. The page below declares no router at all, so BUI chrome has
  // nothing but the framework mount to resolve from — which is the point.
  ...
  it('navigates to the same destination rendered by page-relative BUI links', async () => {
        ...
        loader: async () => (
          <div>
            {targets.map(([label, href]) => (
              <ButtonLink key={label} href={href}>
                {label}
              </ButtonLink>
            ))}
          </div>
    ...
    for (const [label, , destination] of targets) {
      act(() => appHistory.navigate('/catalog/foo'));
      const link = await screen.findByRole('link', { name: label });
      expect(link).toHaveAttribute('href', `/backstage${destination}`);
