...
describe('nfsRoutingDemo', () => {
  it('should run nested React Router v6 routes in a tab that declares the v6 adapter', async () => {
  ...
  it('preserves native and framework links for a tab without an explicit adapter', async () => {
    renderDemoAt('/nfs-routing-demo/unscoped');

    expect(await screen.findByText('No router declared')).toBeInTheDocument();
    // Native routing stays scoped through the compatibility fallback.
    expect(
      screen.getAllByRole('link', { name: 'widget/blue' })[0],
    ).toHaveAttribute('href', '/nfs-routing-demo/unscoped/widget/blue');
    // Framework routing resolves the same target without requiring an adapter.
    expect(
      screen.getAllByRole('link', { name: 'widget/blue' })[1],
    ).toHaveAttribute('href', '/nfs-routing-demo/unscoped/widget/blue');
    expect(
  ...
  it('should let a custom TanStack adapter own one tab while its sibling declares v6', async () => {
  ...
  it('should carry a v6 tab and a v7 tab on one page', async () => {
