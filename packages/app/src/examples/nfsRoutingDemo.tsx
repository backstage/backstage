 ...
 * Every tab below declares the library it wants by rendering that library's
 * page router inside its own `loader`, which is ordinary React and which scopes
  ...
  return (
    <>
      <Typography paragraph>
        ...
        <code style={codeStyle}>loader</code> and owns a{' '}
  ...
  return (
    ...
    >
      <Typography paragraph>
        ...
        <code style={codeStyle}>loader</code> instead, so the two libraries sit
        side by side under one page and neither had to stand down.
      </Typography>
      <Typography paragraph>
...
function UnscopedPanel() {
  ...
  return (
    ...
    >
      <Typography paragraph>
        This tab renders no adapter in its <code style={codeStyle}>loader</code>
        , so it uses the implicit React Router v6 fallback. Its parameters are{' '}
  ...
  return (
    <>
      <Typography paragraph>
        ...
        <code style={codeStyle}>loader</code>, on a page whose other tab
        declares TanStack.
      </Typography>
      <ul>
        <HrefReadout
...
function V7OnlyPanel() {
  return (
    ...
    >
      <Typography paragraph>
        This tab declares React Router v7 in its own{' '}
        <code style={codeStyle}>loader</code>. Its route matches come from v7.
        The app also retains a root v6 context for shared UI; that context has
        no page match. Framework links and both projections observe the same app
        history.
      </Typography>
      <Typography paragraph>
      ...
      <Typography paragraph>
        A <em>relative</em> target works here as well, as long as it is resolved
        by something that reads the page mount. The framework's{' '}
        ...
        app root. A <code style={codeStyle}>react-router-dom</code> v6{' '}
        <code style={codeStyle}>Link</code> would escape, because there is no v6
        route context in this subtree for it to resolve against — which is why
