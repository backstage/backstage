...
describe('AppHistory', () => {
  ...
  it.each([false, true])(
    'should notify once per push, Back and Forward (Navigation API: %s)',
    async useNavigation => {
      history.dispose();
      window.history.replaceState(null, '', '/a');
      const navigation = Object.assign(new EventTarget(), {
        currentEntry: { key: 'a', index: 0 },
      });
      if (useNavigation) {
      ...
      const pushSpy = jest
        ...
        .mockImplementation((state, title, url) => {
          ...
          navigation.currentEntry = { key: 'b', index: 1 };
          navigation.dispatchEvent(new Event('currententrychange'));
        });
      const appHistory = createAppHistory();
      history = appHistory;
      const locations: string[] = [];
      appHistory.location$.subscribe(location =>
        locations.push(location.pathname),
      );
      try {
        appHistory.navigate('/b');
        expect(locations).toEqual(['/a', '/b']);
        for (const [delta, pathname, index] of [
          [-1, '/a', 0],
          [1, '/b', 1],
        ] as const) {
          await new Promise<void>(resolve => {
            window.addEventListener(
              'popstate',
              () => {
                navigation.currentEntry = { key: pathname.slice(1), index };
                navigation.dispatchEvent(new Event('currententrychange'));
                resolve();
              },
              { once: true },
          ...
          expect(appHistory.location.pathname).toBe(pathname);
        }
        expect(locations).toEqual(['/a', '/b', '/a', '/b']);
      } finally {
        pushSpy.mockRestore();
      }
    },
  ...
  it('should release only its own backend subscription when disposed', () => {
    const backend = createMemoryHistoryBackend({
      initialEntries: ['/first', '/second'],
    });
    const first = createAppHistory({ history: backend });
    const second = createAppHistory({ history: backend });
    const firstListener = jest.fn();
    const secondListener = jest.fn();
    const independentListener = jest.fn();
    first.location$.subscribe(firstListener);
    second.location$.subscribe(secondListener);
    const unsubscribe = backend.listen(independentListener);
    ...
    expect(secondListener).toHaveBeenLastCalledWith(
      expect.objectContaining({ pathname: '/first' }),
    );
    expect(independentListener).toHaveBeenCalledTimes(1);
  ...
  describe('createHref', () => {
    ...
    it('should keep a target with no pathname of its own at the current location', () => {
      ...
      expect(history.createHref('#section')).toBe('/catalog/foo/docs#section');
      expect(history.createHref('?tab=readme')).toBe(
        '/catalog/foo/docs?tab=readme',
      );

      history.navigate('/catalog/bar/docs');
      expect(history.createHref('#section')).toBe('/catalog/bar/docs#section');
    });
  });
