...
import {// removeExportStatementFromPlugins,
...
describe('removePlugin', () => {
    describe('Remove Plugin Dependencies', () => {
        ...
        const testPluginPackage = `@spotify-backstage/plugin-${testPluginName}`;
        ...
        it('should remove plugin dependency from /packages/app/package.json', async () => {
            ...
            try {
                ...
                expect(JSON.parse(fse.readFileSync(testFilePath, 'utf8')).hasOwnProperty(testPluginPackage)).toBe(false);
            } finally {
