...
describe('computePgPluginConfig', () => {
  ...
  describe('connection', () => {
    ...
    it('omits database from base connection when pluginDivisionMode is database', () => {
      ...
      expect((result.connection as any).database).toBeUndefined();
    });
    ...
    it('keeps database from base connection when pluginDivisionMode is schema', () => {
      ...
      expect((result.connection as any).database).toBe('shared_db');
    });
    ...
    it('excludes base connection when client is overridden', () => {
      ...
      expect((result.connection as any).host).toBeUndefined();
      expect((result.connection as any).user).toBeUndefined();
      expect((result.connection as any).filename).toBe(':memory:');
    });
  ...
  describe('knexConfig', () => {
    ...
    it('includes additionalKnexConfig properties', () => {
      ...
      expect((result.knexConfig as any).debug).toBe(true);
      expect((result.knexConfig as any).pool).toEqual({ min: 2 });
    });
  });
});
