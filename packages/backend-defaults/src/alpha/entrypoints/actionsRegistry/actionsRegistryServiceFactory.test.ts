...
describe('actionsRegistryServiceFactory', () => {
  ...
  describe('visibilityPermission', () => {
    it('should include visibilityPermission in the list response', async () => {
      ...
      expect(status).toBe(200);
      expect(body.actions[0].visibilityPermission).toEqual({
        name: 'test.action.use',
        attributes: {},
      });
    });

    it('should not include visibilityPermission when not set', async () => {
      const pluginSubject = createBackendPlugin({
        pluginId: 'my-plugin',
        register(reg) {
          reg.registerInit({
      ...
      expect(status).toBe(200);
      expect(body.actions[0].visibilityPermission).toBeUndefined();
    });
  });
});
