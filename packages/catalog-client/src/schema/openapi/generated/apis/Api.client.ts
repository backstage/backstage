...
import { GetEntityRelationsByName200Response } from '../models/GetEntityRelationsByName200Response.model';
 ...
 * @public
 */
export type GetEntityRelationsByName = {
  path: {
    kind: string;
    namespace: string;
    name: string;
...
export class DefaultApiClient {
  ...
  /**
   * Get the relations of an entity by its entity ref.
   * @param kind -
   * @param namespace -
   * @param name -
   * @param relations - A list of relation types to filter by.
   * @param transitive - Whether to include transitive relations.
   */
  public async getEntityRelationsByName(
    // @ts-ignore
    request: GetEntityRelationsByName,
    options?: RequestOptions,
  ): Promise<TypedResponse<GetEntityRelationsByName200Response>> {
    const baseUrl = await this.discoveryApi.getBaseUrl(pluginId);

    const uriTemplate = `/entities/by-name/{kind}/{namespace}/{name}/relations{?relations,transitive}`;
