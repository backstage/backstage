...
describe('RouteLink', () => {
  ...
  it('preserves native targets and downloads while handling self navigation', () => {
    const { rerender } = render(widgetLink, { wrapper });
    for (const props of [
      { target: '_blank' },
      { target: '_parent' },
      { target: '_top' },
      ...
      rerender(cloneElement(widgetLink, props));
      const link = screen.getByRole('link', { name: 'Widget' });
    ...
    rerender(cloneElement(widgetLink, { target: '_self' }));
    const link = screen.getByRole('link', { name: 'Widget' });
    const click = createEvent.click(link);
    fireEvent(link, click);
  ...
  ])('leaves the sanitized external route %s to the browser', (to, href) => {
    render(widgetLink, {
      wrapper: ({ children }: PropsWithChildren) => (
        <TestApiProvider
          apis={[
