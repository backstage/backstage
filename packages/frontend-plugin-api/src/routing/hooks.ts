 ...
 * setSearchParams(new URLSearchParams({ q: 'new query' }));
 * ```
 *
 * @public
 */
...
// These functions are intentionally named to match their react-router-dom
// counterparts (`matchRoutes`, `generatePath`, `resolvePath`) so that
// migration is a simple import swap. They require component context due to
// `useApi()` lookup, which is documented in the `@remarks` JSDoc. Renaming
// to `use*` would break API parity and add friction to the migration path.
