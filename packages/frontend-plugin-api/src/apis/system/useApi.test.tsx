...
describe('useApiHolder', () => {
  ...
  it('should throw when context is present but v1 is not available', () => {
    context.set({ 2: { get: jest.fn() } });

    expect(() => renderHook(() => useApiHolder())).toThrow(
      'ApiContext v1 not available',
    );
  });
});
