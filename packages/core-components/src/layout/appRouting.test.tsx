...
describe('useAppGoBack', () => {
  ...
  it('goes back through the supplied app history', async () => {
    const historyBack = jest.spyOn(window.history, 'back').mockReturnValue();
    const appHistory = createMockAppHistory({ initialLocation: '/one' });
    appHistory.navigate('/two');
    appHistory.navigate('/three');

    function AppLocation() {
      const { pathname } = useAppLocation(appHistory);
      return <p role="status">App location: {pathname}</p>;
    }
    ...
    try {
      ...
      expect(screen.getByRole('heading')).toHaveTextContent('at /three');
