...
export async function buildAppTask(appDir: string) {
  ...
  await Task.forItem('determining', 'yarn version', async () => {
    ...
    if (yarnVersion && !yarnVersion.startsWith('1.')) {
      throw new Error(
        `@backstage/create-app requires Yarn v1, found '${yarnVersion}'. Try running \`yarn set version 1\` as a workaround. You can migrate the project to Yarn 3 after creation using https://backstage.io/docs/tutorials/yarn-migration`,
      );
    }
  });
