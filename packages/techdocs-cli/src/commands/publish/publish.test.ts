...
jest.mock('@backstage/plugin-techdocs-node', () => {
  const mockPublisher = {
    publish: jest.fn().mockResolvedValue({ objects: [] }),
  ...
  return {
    Publisher: {
      fromConfig: jest.fn().mockResolvedValue(mockPublisher),
    },
    __mockPublisher: mockPublisher,
  };
});
...
const mockPublisher = jest.requireMock('@backstage/plugin-techdocs-node')
  .__mockPublisher as {
  publish: jest.Mock;
  getReadiness: jest.Mock;
  fetchTechDocsMetadata: jest.Mock;
};

const mockPublish = mockPublisher.publish;
const mockGetReadiness = mockPublisher.getReadiness;
const mockFetchTechDocsMetadata = mockPublisher.fetchTechDocsMetadata;
