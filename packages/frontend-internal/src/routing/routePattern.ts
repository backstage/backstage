...
/**
 * Path matching and generation shared by app routing and its consumers.
 * Complete branch ranking and compilation belong to frontend-app-api.
 * @internal
 */
...
function compilePath(
  ...
  for (const segment of namedSegments) {
    if (segment.startsWith(':')) {
      ...
      const optional = segment.endsWith('?');
      const required = normalizeSegment(
        optional ? segment.slice(0, -1) : segment,
      );
      regexpSource += optional
...
/**
 * Matches one pattern, keeping URL prefixes in their original spelling.
 * Parameters are decoded once; unbound optional params are omitted so callers
 * can merge ancestry without erasing bound values. Public hooks restore the
 * keys their route refs declare.
 * @internal
 */
export function matchPath(
  ...
  // Normalize each segment independently: encoded slashes stay data inside a
  // segment, while Unicode and escaped route literals compare consistently.
  const normalizedPathname = pathname
    .split('/')
    .map(normalizeSegment)
    .join('/');
  const match = normalizedPathname.match(regexp);
  ...
  // Return prefixes from the original pathname so casing and escaping remain
  // suitable for href generation and for slicing the next nested match.
  const rawPrefix = (normalized: string) =>
    normalized === '/'
      ? '/'
      : pathname.split('/').slice(0, normalized.split('/').length).join('/');
  return {
    matchedPathname: rawPrefix(matchedPathname),
    pathnameBase: rawPrefix(pathnameBase),
    params,
  };
}

/**
 * Drops trailing separators while keeping the leading slash. A linear scan
 * avoids backtracking over long slash runs and also handles line terminators.
 * @internal
 */
...
// Escape percent before slash so literal "%2F" and an encoded slash stay
// distinct, while decoded Unicode still participates in case-insensitive matching.
function normalizeSegment(segment: string): string {
  return safelyDecodeURIComponent(segment)
    .replace(/%/g, '%25')
    .replace(/\//g, '%2F');
}
...
/** Joins a child route below its parent, replacing a parent splat. */
