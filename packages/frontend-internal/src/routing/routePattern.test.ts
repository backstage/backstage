...
describe('routePattern', () => {
  ...
  describe('matchPath', () => {
    ...
    it('omits an optional param the pathname did not bind', () => {
      // The deliberate divergence from react-router, which would report
      // `{ kind: undefined, name: undefined }` here. `params` holds what the
      // location bound, so accumulating matches with `Object.assign` cannot
      // overwrite an outer match's bound value with an inner match's blank.
      // Asserted strictly because `toEqual` ignores undefined-valued keys and
      // so cannot tell the two shapes apart.
      expect(
        matchPath('/catalog/:kind?/:name?', '/catalog', true)?.params,
      ).toStrictEqual({});
      expect(matchPath('/:lang?/about', '/about', true)?.params).toStrictEqual(
        {},
      );

      const accumulated = Object.assign(
        {},
        matchPath('/catalog/:kind', '/catalog/component', false)?.params,
        matchPath('/catalog/:kind?', '/catalog', false)?.params,
      );
      expect(accumulated).toStrictEqual({ kind: 'component' });
    });
    ...
    it('matches empty layout paths as a root base and respects exact matching', () => {
