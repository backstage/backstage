...
export class DevAppBuilder {
   ...
   */
  build(): ComponentType<{}> {
    const sampleRouteRef = createRouteRef({ id: 'sample' });
    const SamplePage = () => <Box p={3}>Page belonging to another plugin.</Box>;
    attachComponentData(SamplePage, 'core.mountPoint', sampleRouteRef);
      ...
      bindRoutes: ({ bind }) => {
        for (const plugin of this.plugins ?? []) {
          ...
          for (const routeKey of Object.keys(plugin.externalRoutes)) {
            targets[routeKey] = sampleRouteRef;
          }
        ...
        <AppProvider>
          ...
          <AppRouter>
            <SidebarPage>
              ...
              <FlatRoutes>
                {this.routes}
                <Route path="/_external_route" element={<SamplePage />} />
              </FlatRoutes>
            </SidebarPage>
          </AppRouter>
        </AppProvider>
