...
describe('PinnipedAuthConnector', () => {
  ...
  it('should refresh a session with default scope', async () => {
    server.use(
    ...
    expect(session.scopes).toEqual(new Set(['default-scope']));
  ...
  it('should create a session', async () => {
    ...
    expect(popupSpy.mock.calls[0][0]).toMatchObject({
      url: 'http://my-host/api/auth/my-provider/start?audience=555-444-333-2-1&origin=http%3A%2F%2Flocalhost&flow=popup&env=production',
    });
