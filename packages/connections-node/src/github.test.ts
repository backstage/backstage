...
describe('matchAuth', () => {
  it('picks the app whose orgs contains the URL org', async () => {
    ...
    const acme = await catalog.find({
      type: 'github',
      params: { url: 'https://matchauth.example.com/acme/repo' },
      authMethods: ['app'],
    });
    const widgets = await catalog.find({
      type: 'github',
      params: { url: 'https://matchauth.example.com/widgets/other-repo' },
      authMethods: ['app'],
    });
  ...
  it('falls back to the unrestricted app when no orgs matches', async () => {
    ...
    const connection = await service.forPlugin('catalog').find({
      type: 'github',
      params: { url: 'https://matchauth.example.com/unknown-org/repo' },
      authMethods: ['app'],
    });
